...
| 영역 | 현재 결정 |
| --- | --- |
| 배포 범위 | `0.3.0`은 local-first, single-project, single-user다. 기본 bind는 `127.0.0.1:4319`이며 login이 없다. |
| package | Python SDK는 PyPI에, collector image는 GHCR에 `0.3.0` artifact로 공개한다. public 문서는 실제 artifact install/pull로 확인한 version만 안내한다. |
