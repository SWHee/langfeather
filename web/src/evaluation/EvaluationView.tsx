...
> = {
  input: (example) => jsonPreview(example.input),
  expected_output: (example) => jsonPreview(example.expected_output),
...
export function EvaluationView({
  ...
  const activeTab = state.tab === "examples" ? "examples" : "experiments";
  ...
  const changeTab = (tab: "examples" | "experiments") =>
    onChange({ ...state, tab, caseId: null });
  ...
  const onTabsKeyDown = (event: ReactKeyboardEvent<HTMLButtonElement>) => {
    const tabs: Array<"examples" | "experiments"> = ["examples", "experiments"];
    ...
    <main className="page evaluation-page">
      {loadState === "loading" ? (
        ...
        <section className="dataset-detail-view">
          <header className="detail-head evaluation-detail-head">
            <button
              className="back"
              type="button"
              onClick={() =>
            ...
            {pagedVisible.length === 0 ? (
              ...
              pagedVisible.map((example) => {
                const cell: Record<string, ReactNode> = {
                  input: (
                    <span className="json">{jsonPreview(example.input)}</span>
                  ),
                  expected_output: (
                    <span className="json">
                      {jsonPreview(example.expected_output)}
                    </span>
                  ),
                  metadata: (
                    <span className="json">
...
function ExperimentCaseTable({ experiment }: { experiment: Experiment }) {
  ...
  >(() => {
    ...
    > = {
      input: (experimentCase) => jsonPreview(experimentCase.input),
      expected_output: (experimentCase) =>
        jsonPreview(experimentCase.expected_output),
      output: (experimentCase) => jsonPreview(experimentCase.output),
      ...
      <table>
        ...
        <tbody>
          {sortedCases.map((experimentCase) => {
            const cell: Record<string, ReactNode> = {
              input: jsonPreview(experimentCase.input),
              expected_output: jsonPreview(experimentCase.expected_output),
              output: jsonPreview(experimentCase.output),
