...
/**
 * 정규화 기준으로 `count`자를 소비했을 때 원문의 어느 index인지. normalize가
 * 공백 run을 한 칸으로 접으므로 그 접힌 만큼을 되짚어 센다.
 */
function originalIndexAfter(text: string, count: number): number {
  const isSpace = (character: string) => /\s/.test(character);
  let index = 0;
  let produced = 0;
  // normalize가 앞쪽 공백을 잘라내므로 여기서도 건너뛴다.
  while (index < text.length && isSpace(text[index])) index += 1;
  while (index < text.length && produced < count) {
...
/**
 * 문서 text의 앞부분 중 하류 llm input에 그대로 실린 구간의 끝 index(원문 기준).
 * 실리지 않았으면 0이다. 화면은 이 구간만 하이라이트한다 — 어림잡아 칠하지
 * 않고 실제로 대조된 만큼만 표시한다.
 */
export function matchedPrefixEnd(
  document: RetrievedDocument,
  llmInput: string,
): number {
  if (!usedInAnswer(document, llmInput)) return 0;
  const haystack = normalize(llmInput);
  const needle = normalize(document.text);
  // 포함 여부는 길이에 대해 단조 감소하므로 이분 탐색으로 가장 긴 접두사를 찾는다.
  let low = 0;
  ...
  if (low === 0) return 0;
  return originalIndexAfter(document.text, low);
}
